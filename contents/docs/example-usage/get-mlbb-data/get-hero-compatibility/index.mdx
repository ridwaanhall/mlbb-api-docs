...
description: API endpoint uses hero id or hero name (1-130 (still update until now)) to fetch hero compatibility data.
---

## Endpoint

```txt
GET /api/hero-compatibility/<str:heroid_or_name>/
```

## Path Parameters

1. **heroid_or_name**
   The ID or name of the main hero whose compatibility information is to be fetched.

   - **Possible Values:** From `1` to `130` (still update until now) or with hero name like Luo Yi its luoyi
   - **Type:** integer
   - **Required:** `True`
